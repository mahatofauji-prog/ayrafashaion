  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#F5F5F5] pb-24">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8">
        ...
        <HeroCarousel banners={banners || []} businessProfile={businessProfile} isLoading={isLoading} />
            ...
            <div className="flex items-center space-x-2 overflow-x-auto pb-2 scrollbar-none max-w-full">
              ...
              {categories.map((cat) => {
                ...
                return (
                  ...
                  >
                    {cat.name.toUpperCase()} {isLoading ? '' : `(${count})`}
                  </button>
                );
              })}
            </div>
        ...
        <div className="flex items-center justify-between mb-6">
          {isLoading ? (
            <div className="flex items-center space-x-2 text-xs uppercase tracking-widest font-semibold text-zinc-500 animate-pulse">
              <RefreshCw className="w-3 h-3 text-[#D4AF37] animate-spin" />
              <span>Fetching premium collection...</span>
            </div>
          ) : (
        ...
        {hasError ? (
          <div className="bg-[#0E0E0E] rounded-3xl border border-rose-950/80 p-12 text-center max-w-md mx-auto my-12 space-y-4 shadow-xl">
            <div className="w-16 h-16 rounded-full bg-rose-950/40 border border-rose-800/50 flex items-center justify-center mx-auto text-rose-400">
              <AlertTriangle className="w-8 h-8" />
            </div>
            <h3 className="text-lg font-serif font-bold text-white">Failed to load products</h3>
            <p className="text-xs text-zinc-400 leading-relaxed">
              We encountered a connection issue while sync-fetching the catalog from the servers. Please check your network connection and try again.
            </p>
            <button
              onClick={onRetry}
              className="px-6 py-2.5 bg-[#D4AF37] hover:bg-[#C9A227] text-black font-extrabold uppercase tracking-wider rounded-xl text-xs transition-colors flex items-center justify-center space-x-2 mx-auto shadow-md"
            >
          ...
          /* Premium Product Skeletons */
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-6">
            {Array.from({ length: 8 }).map((_, idx) => (
              <div
                key={`skeleton-${idx}`}
                className="bg-[#0D0D0D] rounded-2xl border border-zinc-800/60 overflow-hidden shadow-sm flex flex-col h-full animate-pulse"
              >
                <div className="relative aspect-4/5 w-full bg-zinc-900/65" />
                <div className="p-3 sm:p-4 flex-1 flex flex-col justify-between space-y-4">
                  <div>
                    <div className="flex flex-wrap items-center gap-1.5 mb-2">
                      <div className="h-4 w-12 bg-zinc-850 rounded-md" />
                      <div className="h-4 w-16 bg-zinc-850 rounded-md" />
                    </div>
                    <div className="h-4 w-3/4 bg-zinc-850 rounded-md" />
                    <div className="h-3 w-5/6 bg-zinc-900 rounded-md mt-2" />
                  </div>
                  <div className="pt-2.5 border-t border-zinc-850 flex flex-col gap-2">
                    <div className="space-y-1">
                      <div className="h-2 w-16 bg-zinc-900 rounded" />
                      <div className="h-5 w-24 bg-zinc-850 rounded" />
                    </div>
                    <div className="h-9 w-full bg-zinc-850/80 rounded-xl mt-1" />
                  </div>
                </div>
              </div>
            ))}
      ...
      <footer className="mt-20 border-t border-[#D4AF37]/20 bg-[#070707] py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
          <div className="flex flex-col space-y-3 items-center md:items-start">
            ...
            <div className="flex flex-col space-y-1.5 text-xs sm:text-sm text-zinc-350 font-medium">
              <p className="text-[#D4AF37] font-bold tracking-wide">✨ Where Fashion Meets Trend</p>
        ...
        <div className="max-w-7xl mx-auto mt-8 pt-6 border-t border-zinc-900 text-center text-[11px] text-zinc-600 flex flex-col items-center justify-center space-y-1.5">
          <p>© {new Date().getFullYear()} {businessProfile.businessName}. All rights reserved. Premium Digital Catalogue Software.</p>
          <p className="text-[10px] text-zinc-500 font-medium">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="text-[#B8860B] hover:text-[#D4AF37] font-semibold transition-colors duration-200 underline decoration-zinc-800/50 hover:decoration-[#D4AF37]/50 underline-offset-2"
            >
              MANI Solution
            </a>
          </p>
